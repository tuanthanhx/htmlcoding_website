...
app.tabletViewport = function() {
  ...
  if (app.isMobile() && $('#fullpage').length) {
...
app.fullpage = function() {
  if (!app.isMobile() || !$('#fullpage').length) return;

  new fullpage('#fullpage', {
    licenseKey: '0751B3F1-66C041AD-8CBA5EEB-13391333',
    autoScrolling: true,
    scrollOverflow: app.isMobile() ? true : false,
...
app.animationProject = function() {
  ...
  if (app.isMobile() || !$('#particles-js').length) return;
...
app.animationAstronaut = function() {
  ...
  if (!$('.section-astronaut').length) return;
